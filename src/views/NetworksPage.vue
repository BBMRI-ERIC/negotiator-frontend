<template>
  ...
  onMounted(async () => {
    ...
    if (Object.keys(route?.query).length) {
      ...
      await store.dispatch("retrieveUserNetworks", { userId: userId.value }).then((res) => {
  ...
  async function retrieveNetworkNegotiationsById () {
    negotiations.value = await store.dispatch("retrieveNetworkNegotiations", {
        networkId: userNetworksSelectedData.value,
    })
