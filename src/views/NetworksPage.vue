...
if (Object.keys(userStore.userInfo).length === 0) {
  ...
  if (Object.keys(route?.query).length) {
    ...
    networksPageStore.retrieveUserNetworks(userId.value).then((res) => {
...
async function retrieveNetworkNegotiationsById () {
    negotiations.value = await networksPageStore.retrieveNetworkNegotiations(userNetworksSelectedData.value)
