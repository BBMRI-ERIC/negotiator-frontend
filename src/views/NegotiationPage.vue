    ...
    <div v-if="this.negotiation.status=='APPROVED'">
            ...
            <ul  class="list-group list-group-vertical "  v-for="post in posts" :key="post.text">
                ...
                <li class="list-group-item d-flex justify-content-between align-items-center"> {{ post.text }}
                    <div>
                        <span v-if="post.status=='CREATED'&&post.poster_role!=this.userRole" class="badge bg-primary rounded-pill">New</span>
                        <button v-if="post.status=='CREATED'&&post.poster_role!=this.userRole" @click="updateMessageStatus(post.id, post.text)" type="submit" class="btn btn-secondary btn-sm">Mark as read</button>
                    </div>
                </li>
...
export default {
  ...
  methods: {
    ...
    async created() {
        ...
        console.log(this.negotiation.requests[0].resources[0].id)
